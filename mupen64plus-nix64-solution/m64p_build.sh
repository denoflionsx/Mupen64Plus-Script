...
if [ -z "$M64P_COMPONENTS" ]; then
	M64P_COMPONENTS="core binding-shell ui-console audio-sdl input-sdl rsp-hle video-rice video-glide64mk2"
fi
...
for component in ${M64P_COMPONENTS}; do
	...
	#"$MAKE" -C ../mupen64plus-${component}/projects/unix install $@ ${MAKE_INSTALL} DESTDIR="$(pwd)/test/"
done
